...
如果把后面这些方法抽象一下，它们反复在解决的其实是同一组训练瓶颈，而不是彼此完全不同的问题。总的来说，可以概括成两类：

-   **信号问题**：奖励太晚、太粗、太弱，导致不知道该把功劳分给哪一步。
-   **优化问题**：就算有一些信号，训练过程也很容易高方差、退化、坍塌，导致学不动。

更具体一点，共性问题基本集中在下面这些方面：

-   **延迟稀疏奖励**：很多 agent 任务只有整条轨迹结束才拿到 outcome reward，中间 turn 没有即时反馈，所以第一轮一个关键搜索、一次关键工具调用，到底该拿多少 credit 很难判断。
-   **credit 粒度过粗**：标准做法常把整条轨迹奖励均摊到所有 token，或者最多只做到 episode 级比较，导致关键动作和无关动作混在一起，无法区分到底哪个 turn、哪个 step 真正推动了成功。
-   **缺少可比性**：想判断一步动作好不好，前提是有参照物；但开放环境里状态多样、动作空间很大，不同 rollout 也很难恰好落到可比较的同一状态，因此 advantage 很难估。
-   **探索不足或探索质量差**：如果采样出的轨迹不够多样，中间状态就没有区分度，credit 信号也无从产生；很多方法本质上都在想办法让 rollout 更有区分性。
-   **方差大、baseline 退化、梯度消失**：多轮任务轨迹长、奖励稀疏，`GRPO`/`PPO` 很容易出现组内方差塌缩、advantage 接近零、critic 不稳定、梯度传不回去的问题。
-   **token 级 RL 与多轮 agent 结构不匹配**：标准 `PPO`/`GRPO` 更像在优化 token 序列，但 agent 任务里的真实决策单位往往是 turn、tool call、search action；轮内和轮间的状态转移机制混在一起，会让 value 和 advantage 失真。
-   **环境重放和额外 rollout 成本高**：越精确的 credit assignment 往往越依赖从中间状态重模拟或做反事实比较，但在 web/search/tool agent 里这类操作代价很高，所以很多方法都在试图用更便宜的近似去替代。
-   **优化机制会压制真正该学习的位置**：像高熵工具调用 token 这类最不确定、最需要学习的地方，反而更容易在采样和 `PPO clip` 中被预算耗尽或梯度截断，形成“越不确定的地方越学不到”的恶性循环。

一句话概括：这些方法共同想解决的是，在**长轨迹、稀疏奖励、开放动作空间**的 agent RL 中，如何用有限采样和可接受的计算成本，稳定地判断每一步到底贡献了什么，并把梯度真正传回那些关键但不确定的决策点。
